...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const tabJuniorContainer = document.querySelector(".tab-container-junior");
    // for girls
    const tabJuniorGirlContainerElm = document.querySelector(
        ".tab-junior-girls-container"
    );
    const tabJuniorGirlContainerBtnElm = document.querySelectorAll(
        "tab-junior-girl-container-btn"
    );
    // for boys
    const dropdownUpcomingTournamentsElm = document.getElementById(
        "dropdownUpcomingTournaments"
    );
    const boysJuniorPlayerBtnElm = document.getElementById(
        "boysJuniorPlayerBtn"
    );
    const tabsJunior = document.querySelectorAll(".tab-junior-container-btn");
    const tabs = document.querySelectorAll(".tab-container-btn");
    const tabItemContents = document.querySelectorAll(".tab-item-col");
    ...
    if (tabContainer) {
        tabContainer.addEventListener("click", (event) => {
            const clicked = event.target.closest(".tab-container-btn");
            if (!clicked) return;
            ...
            document
                .querySelector(`.tab-item-col__${clicked.dataset.filter}`)
                .classList.add("active");
        });
    }
    // for girl container
    if (tabJuniorGirlContainerElm) {
        tabJuniorGirlContainerElm.addEventListener("click", (event) => {
            event.preventDefault();
            const clicked = event.target.closest(
                ".tab-junior-girl-container-btn"
            );
            if (!clicked) return;
            tabJuniorGirlContainerBtnElm.forEach((btn) =>
    ...
    // for boys container
    if (dropdownUpcomingTournamentsElm) {
        dropdownUpcomingTournamentsElm.addEventListener("click", (event) => {
            event.stopPropagation();
            boysJuniorPlayerBtnElm.classList.toggle("show");
        });
    }
    ...
    //for junior players in upcoming tournaments
    if (tabJuniorContainer) {
        tabJuniorContainer.addEventListener("click", (event) => {
            // console.log("event", event)
            const clicked = event.target.closest(".tab-junior-container-btn");

            if (!clicked) return;
            tabsJunior.forEach((tab) => tab.classList.remove("active"));
            // console.log("clicked btn is: ", clicked)
            clicked.classList.add("active");
            // content active
            tabItemContents.forEach((content) =>
                content.classList.remove("active")
    ...
    // for top ranking tab player functionality
    if (recentResultBtnContainer) {
        recentResultBtnContainer.addEventListener("click", (event) => {
            const clicked = event.target.closest(".recent-result-btn");
            console.log("clicked is: ", clicked);
            if (!clicked) return;
            recentResultPlayerBtn.forEach((btn) =>
                btn.classList.remove("active")
            );
            clicked.classList.add("active");
            recentResult.forEach((btn) =>
                btn.classList.remove("recent-result-active")
            );
            document
                .querySelector(`.recent-result-${clicked.dataset.tab}`)
                .classList.add("recent-result-active");
        });
    }

    // for top ranking tab junior player functionality
    if (recentResultBtnJuniorContainer) {
        recentResultBtnJuniorContainer.addEventListener("click", (event) => {
            const clicked = event.target.closest(
                ".recent-result-junior-player-btn"
            );
            console.log("clicked is: ", clicked);
            if (!clicked) return;
            recentResultJPlayerBtn.forEach((btn) =>
                btn.classList.remove("active")
            );
            clicked.classList.add("active");
            recentResult.forEach((btn) =>
